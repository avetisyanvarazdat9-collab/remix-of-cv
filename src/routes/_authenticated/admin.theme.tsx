...
const PRESETS: { name: string; tag: "Dark" | "Light" | "Tech"; colors: Omit<Colors, "theme_mode"> }[] = [
  // Dark
...
function presetThemeMode(tag: "Dark" | "Light" | "Tech"): ThemeMode {
  return tag === "Light" ? "light" : "dark";
}
...
function ThemeEditor() {
  ...
  const fields: { key: keyof Omit<Colors, "theme_mode">; label: string; hint: string }[] = [
  ...
  const otherMode = colors.theme_mode === "light" ? "dark" : "light";

  return (
    <div>
      <h1 className="font-display text-3xl font-bold">Theme</h1>
      <p className="mt-1 text-muted-foreground">
        Pick colors or a preset. Custom colors apply only in the mode you choose; the other mode uses the site defaults.
      </p>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="glass rounded-2xl p-6">
          ...
          <div className="mt-6">
            <p className="text-xs uppercase tracking-wider text-muted-foreground">Applies in visitor mode</p>
            <div className="mt-2 flex gap-2">
              ...
              <button
                type="button"
                onClick={() => patch("theme_mode", inferThemeModeFromBackground(colors.background_color))}
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Visitors in {colors.theme_mode} mode see these colors. In {otherMode} mode they see the default site theme.
            </p>
          </div>
            ...
            {PRESETS.map((p) => (
              <button
                ...
                onClick={() =>
                  setColors({
                    ...p.colors,
                    theme_mode: presetThemeMode(p.tag),
                  })
                }
        ...
        <div>
          ...
          <p className="mt-3 text-xs leading-relaxed text-muted-foreground">
            This palette applies when visitors use <span className="font-medium text-foreground">{colors.theme_mode} mode</span>.
            In <span className="font-medium text-foreground">{otherMode} mode</span>, they will see the site&apos;s default {otherMode} theme from styles.css — not these custom colors.
          </p>
        </div>
      </div>
    </div>
